...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Edit Kurir',
        href: '/admin/users-management/couriers/edit',
    },
];

export default function EditPage({ courier }: { courier: Courier }) {
    ...
    const { data, setData, processing, errors, reset } = useForm<Required<CourierForm>>({
        ...
        hired_at: new Date(courier?.hired_at ?? ''),
        stopped_at: new Date(courier?.stopped_at ?? ''),
        salary: courier?.salary,
        gender: courier?.gender,
        shift: courier?.shift,
        status: courier?.status,
        job_type: courier?.job_type,
    });
    ...
    const handleSubmit: FormEventHandler<HTMLFormElement> = (event) => {
        ...
        router.put(route('admin.couriers.update', { id: courier?.id }), formattedData, {
            onSuccess: () => {
                toast.success('Success', {
                    description: 'Kurir Berhasil Diedit!',
                    action: {
                        label: 'Tutup',
            ...
            onError: (errors) => {
                toast.error('Failed', {
                    description: errors.message || 'Gagal Mengedit Kurir',
                    action: {
                        label: 'Tutup',
                        onClick: () => toast.dismiss(),
                    },
        ...
        <>
            <AdminLayout breadcrumbs={breadcrumbs}>
                ...
                <form onSubmit={handleSubmit} className="space-y-5 p-4">
                    <div id="user_id">
                        ...
                        <Select onValueChange={(value) => setData('user_id', parseInt(value))}>
                            <SelectTrigger className="mt-2 w-full" disabled>
                                <SelectValue
                                    placeholder={
                                        usersCourier.find((item: User) => item.id === data.user_id)
                                            ? `${usersCourier.find((item: User) => item.id === data.user_id)?.full_name} - ${usersCourier.find((item: User) => item.id === data.user_id)?.phone_number}`
                                            : 'Pilih Kurir'
                                    }
                                />
                            </SelectTrigger>
                            <SelectContent>
                                {usersCourier.map((item: User) => (
                    ...
                    <div id="status">
                        <Label htmlFor="status">Status Kurir</Label>
