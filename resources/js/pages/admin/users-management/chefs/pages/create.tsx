...
import { ChefForm } from '@/models/chef';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Tambah Koki',
        href: '/admin/users-management/chefs/create',
    },
];

export default function CreatePage() {
    const { usersChef } = usePage<{ usersChef: User[] }>().props;
    const { existingChefs } = usePage<{ existingChefs: number[] }>().props;
    const availableUsers = usersChef.filter((user) => !existingChefs.includes(user.id));
        ...
        router.post(route('admin.chefs.store'), formattedData, {
            onSuccess: () => {
                toast.success('Success', {
                    description: 'Koki Berhasil Ditambahkan!',
                    action: {
                        label: 'Tutup',
            ...
            onError: (errors) => {
                toast.error('Failed', {
                    description: errors.user_id || 'Gagal Menambahkan Koki',
                    action: {
                        label: 'Tutup',
                        onClick: () => toast.dismiss(),
                    },
