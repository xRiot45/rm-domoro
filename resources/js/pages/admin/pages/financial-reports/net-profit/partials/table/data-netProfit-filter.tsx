...
interface NetProfitRangeFilterProps<TData, TValue> {
    column: Column<TData, TValue>;
    title?: string;
}

export function NetProfitRangeFilter<TData, TValue>({ column, title = 'Tanggal' }: NetProfitRangeFilterProps<TData, TValue>) {
        ...
        <Popover>
            <PopoverTrigger>
                <Button variant="outline" className="w-[350px] justify-start text-left font-normal">
                    ...
                    {range?.min || range?.max ? `Laba bersih: ${formatCurrency(range?.min ?? 0)} - ${formatCurrency(range?.max ?? 0)}` : title}
                </Button>
            </PopoverTrigger>
            <PopoverContent className="z-50 mt-3 w-[350px] space-y-3">
                <div className="flex flex-col space-y-2">
                    ...
                    <Input
                        type="number"
                        placeholder="Minimal laba bersih"
                        value={range?.min ?? ''}
                        onChange={(e) => {
                ...
                <div className="flex flex-col space-y-2">
                    ...
                    <Input
                        type="number"
                        placeholder="Maksimal laba bersih"
                        value={range?.max ?? ''}
                        onChange={(e) => {
                            const value = e.target.value;
                            setRange((prev) => ({
