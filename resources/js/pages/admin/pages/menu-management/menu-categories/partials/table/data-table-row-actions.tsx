...
export function DataTableRowActions({ row }: { row: Row<MenuCategory> }) {
    const handleDelete = (menuCategoryId: number) => {
        router.delete(route('admin.menu-categories.destroy', { id: menuCategoryId }), {
            onSuccess: () => {
            ...
            onError: (errors) => {
                toast.error('Failed', {
                    description: errors.message || 'Gagal Menghapus Kategori Menu',
                    action: {
                        label: 'Tutup',
                        onClick: () => toast.dismiss(),
                    },
        ...
        <>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-[160px]">
                    <Link href={route('admin.menu-categories.edit', { id: row.original.id })} className="cursor-po">
