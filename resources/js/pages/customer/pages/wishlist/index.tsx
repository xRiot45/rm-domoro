...
export default function WishlistPage({ wishlists }: { wishlists: Wishlist[] }) {
    const { menuCategories } = usePage<{ menuCategories: MenuCategory[] }>().props;
    const [searchTerm, setSearchTerm] = useState<string>('');
    const [selectedCategory, setSelectedCategory] = useState<string | null>(null);

    const menuItems: MenuItems[] = wishlists.map((wishlist) => wishlist.menu_item);
        ...
        <>
            <AppLayout>
                ...
                <div className="my-8 flex flex-wrap justify-between space-y-2">
                    <div>
                        <h2 className="text-2xl font-black tracking-tight text-gray-700 dark:text-gray-200">Daftar Wishlist Anda</h2>
                        <p className="text-muted-foreground mt-1.5 text-[14px]">Kelola menu yang ada di dalam wishlist anda</p>
                    </div>
                </div>
